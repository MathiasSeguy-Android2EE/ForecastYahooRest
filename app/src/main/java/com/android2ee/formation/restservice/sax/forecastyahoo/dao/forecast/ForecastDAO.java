 ...
 * 25 mars 2011</br>
 */
package com.android2ee.formation.restservice.sax.forecastyahoo.dao.forecast;
...
public class ForecastDAO {
	 ...
	 * @return
	 */
	public ArrayList<YahooForcast> loadAll(String woeid) {
		openDB();
		...
		String where = Constants.KEY_COL_WOEID+"=?";
		String[] whereParam=new String[] {woeid};
		// Maximal size of the results list
		Cursor cursor = db.query(Constants.MY_TABLE, projections, where, whereParam, null, null, orderBy,
				null);
	 ...
	 */
	public boolean saveAll(ArrayList<YahooForcast> forcasts,String woeid) {
		boolean insertionOk=true;
		Collections.sort(forcasts, new YahooForecastComparator());
		//As they are sorted I can store them as this
		openDB();
		//first flush data associated with that woeid
		String where = Constants.KEY_COL_WOEID+"=?";
		String[] whereParam=new String[] {woeid};
		db.delete(Constants.MY_TABLE, where, whereParam);
		//then save all
		...
		for(YahooForcast forcast:forcasts) {
			...
			contentValues.put(Constants.KEY_COL_WOEID, woeid);
